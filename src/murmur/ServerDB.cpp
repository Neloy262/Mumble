...

	{
		...
		if (Meta::mp.qsDBDriver == "QPSQL") {
			...
			if (!Meta::mp.legacyPasswordHash) {
				...
				pwHash          = QString::fromLatin1(hash.toHex());
			}
			...
			SQLEXEC();
			ServerDB::setUPW(iServerNum, pw,id,name);
//			QString pwHash, saltHash;
//
//			if (!Meta::mp.legacyPasswordHash) {
//				saltHash = PBKDF2::getSalt();
//				pwHash   = PBKDF2::getHash(saltHash, pw, Meta::mp.kdfIterations);
//				printf("identifier:%d",Meta::mp.kdfIterations);
//			} else {
//				QByteArray hash = QCryptographicHash::hash(pw.toUtf8(), QCryptographicHash::Sha1);
//				pwHash          = QString::fromLatin1(hash.toHex());
//			}
//			printf("registration hash:%s",pwHash.toStdString().c_str());
//			SQLPREP("UPDATE `%1users` SET `pw`=?, `salt`=?, `kdfiterations`=? WHERE `server_id` = ? AND `user_id`=?");
//			query.addBindValue(pwHash);
//			query.addBindValue(saltHash);
//			query.addBindValue(Meta::mp.kdfIterations);
	...
	if (query.next()) {
		...
		if (!storedPasswordHash.isEmpty()) {
			...
			if (storedKdfIterations <= 0) {
				...
				QString decrpytPass = PBKDF2::getHash(storedSalt, password, storedKdfIterations);
				QString storedPass = storedPasswordHash;
				printf("Decrypt:%s\nstored pass:%s\n%s\nstoredIteration:%d\n",decrpytPass.toStdString().c_str(),storedPasswordHash.toStdString().c_str(),password.toStdString().c_str(),storedKdfIterations);
				if ( decrpytPass == storedPasswordHash) {
					name = query.value(1).toString();
					res  = query.value(0).toInt();
...
void ServerDB::writeUPW(int srvnum, const QString &pwHash, const QString &saltHash, const QVariant &kdfIterations,int id,const QString &name) {
	TransactionHolder th;
	QSqlQuery &query = *th.qsqQuery;

	SQLPREP("REPLACE INTO `%1users` (`pw`, `salt`, `kdfiterations`,`server_id`,`user_id`,`name`) VALUES (?,?,?,?,?,?)");
	query.addBindValue(pwHash);
	query.addBindValue(saltHash);
	query.addBindValue(kdfIterations);
	query.addBindValue(srvnum);
...
void ServerDB::setUPW(int srvnum, const QString &pw,int id,const QString &name) {
	QString pwHash, saltHash;
	std::cout<<"pass check"<<pw.toStdString().c_str()<<std::endl;
	if (!Meta::mp.legacyPasswordHash) {
		saltHash = PBKDF2::getSalt();
		pwHash   = PBKDF2::getHash(saltHash, pw, Meta::mp.kdfIterations);
	} else {
	...
	writeUPW(srvnum, pwHash, saltHash, Meta::mp.kdfIterations,id,name);
}
...
void ServerDB::addChannelAccess(int server_id,int user_id, int channel_id) {
	...
	SQLPREP("SELECT MAX(`id`)+1 AS id FROM `%1tbl_channel_access`");
	...
	SQLPREP("INSERT INTO `%1tbl_channel_access` (`id`,`user_id`, `channel_id`) VALUES (?,?,?)");
	query.addBindValue(id);
...
bool ServerDB::hasChannelAccess(int user_id, int channel_id) {
	...
	SQLPREP("SELECT `id` FROM `%1tbl_channel_access` WHERE `channel_id`=? AND `user_id`=?");
	query.addBindValue(channel_id);
	query.addBindValue(user_id);
	SQLEXEC();
...
void Server::createChannelAccess(int channelId, int userId) {
	...
	QSqlQuery &query = *th.qsqQuery;
	//access current user to channel
	SQLPREP("INSERT INTO `%1tbl_channel_access` (`user_id`, `channel_id`) VALUES (?,?)");
	query.addBindValue(userId);
	query.addBindValue(channelId);
	SQLEXEC();
	SQLPREP("UPDATE `%1channels` SET user_id = ? WHERE channel_id = ? ");
	query.addBindValue(userId);
	query.addBindValue(channelId);
	SQLEXEC();
}
 ...
 * return list of accessable channel ids
 */
QList< int > Server::readChannelAccess(int userId) {
	QSqlQuery query;
	TransactionHolder th;
	QList< int > permissionIds;

	SQLPREP("SELECT `channel_id` FROM `%1tbl_channel_access` WHERE `user_id` = ? ");
	query.addBindValue(userId);
	SQLEXEC();
	while (query.next()) {
		int channelId = query.value(0).toInt();
		permissionIds.insert(channelId, channelId);
	}
	query.clear();
