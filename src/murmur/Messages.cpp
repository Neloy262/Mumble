...
void Server::msgAuthenticate(ServerUser *uSource, MumbleProto::Authenticate &msg) {
	...
	q << root;
	MumbleProto::ChannelState mpcs;
	QList<int> accessIds = readChannelAccess(uSource->iId);
	foreach(id,accessIds)
	{
		printf("Channel IDs:%d\n",id);
	}
	while (!q.isEmpty()) {
		c = q.dequeue();
		chans.insert(c);
		...
		/*if(accessIds.value(mpcs.channel_id())>0 || mpcs.channel_id()==0)
		{
...
void Server::msgUserState(ServerUser *uSource, MumbleProto::UserState &msg) {
	...
	*/
	...
	if (msg.has_user_id()) {
		...
		info.insert(ServerDB::User_Password, "1234");
...
void Server::msgChannelState(ServerUser *uSource, MumbleProto::ChannelState &msg) {
	...
	if (!c) {
		...
		//now any user can create channel, removed restrictions
		...
		int user_id = uSource->iId;
		printf("####userid:%d",user_id);
		c = addChannel(p, qsName, msg.temporary(), msg.position(), msg.max_users(),user_id);
		if (!c->bTemporary) {

			Server::createChannelAccess(c->iId,uSource->iId);
		}
//		if(!msg.temporary())
//		{
//			createChannelAccess(c->iId,user_id);
//		}
...
/*void Server::msgUserInvitation(ServerUser *uSource, MumbleProto::UserInvitation &msg){
	printf("invitation msg received...");
	MSG_SETUP(ServerUser::Authenticated);
	QMutexLocker qml(&qmCache);
